/*
 * YouMe Transformer Embedded JavaScript Standard Library Definition
 *
 * This is a host-side code-generation tool. It is compiled and run on
 * the build host to produce include/embedded_stdlib.h, which is then
 * compiled into the firmware as ROM data.
 *
 * It is based on lib/mquickjs/mqjs_stdlib.c but omits APIs that are not
 * available on embedded targets: load(), setTimeout(), clearTimeout().
 *
 * To regenerate include/embedded_stdlib.h, run from the repo root:
