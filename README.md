...
To use Micro QuickJS, two headers need to be dynamically generated on the host system prior to compiling the Micro QuickJS source code:
...
2. `embedded_stdlib.h` is generated from `src/embedded_stdlib.c`, which defines the JavaScript standard library surface available at runtime on this project. It must be regenerated whenever `src/embedded_stdlib.c` is changed or the mquickjs submodule is upgraded.

For convenience, this header has been pre-generated and committed to the project's `include/` directory.

To generate the headers, run: 

```sh
./generate-js-headers.sh
```
