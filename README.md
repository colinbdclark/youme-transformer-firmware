...
When you first clone this repository, you'll need to initialize and update all submodules:
...
### Generating the Micro QuickJS Headers

To use Micro QuickJS, two headers are dynamically generated on the host system prior to compiling the firmware:

1. `mquickjs_atom.h` is generated by the `mqjs_stdlib` tool in the Micro QuickJS submodule. It contains integer offsets into the engine's internal string table and must be regenerated whenever the mquickjs submodule is upgraded.
2. `js-stdlib-table.h` is generated from `src/code-generation/generate-js-stdlib.c`, which defines the JavaScript standard library surface available at runtime on this project. It must be regenerated whenever `src/code-generation/generate-js-stdlib.c` is changed or the mquickjs submodule is upgraded.

Both headers have been pre-generated and committed to the project's `include/` directory. To regenerate them, run the CMake build.
